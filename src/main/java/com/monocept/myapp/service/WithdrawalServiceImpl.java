...
//	}
    ...
    public PagedResponse<WithdrawalResponseDto> getCommissionWithdrawalsWithFilters(int page, int size, String sortBy, String direction, 
        ...
        Page<WithdrawalRequest> withdrawalPage = withdrawalRepository.findCommissionWithdrawals(agentId, status, fromDateTime, toDateTime, pageRequest);
	...
	public PagedResponse<WithdrawalResponseDto> getCommissionWithdrawalsWithFilters(int page, int size, String sortBy,
		...
		LocalDateTime fromDateTime = (fromDate != null) ? fromDate.atStartOfDay() : null;
	    LocalDateTime toDateTime = (toDate != null) ? toDate.atTime(23, 59) : null;
        Page<WithdrawalRequest> withdrawalPage = withdrawalRepository.findCommissionWithdrawals(getAgentFromSecurityContext().getAgentId(), status, fromDateTime, toDateTime, pageRequest);
        List<WithdrawalResponseDto> withdrawals = withdrawalPage.getContent().stream().map(withdrawalRequest->convertWithdrawalToDto(withdrawalRequest)).collect(Collectors.toList());
        return new PagedResponse<>(withdrawals, withdrawalPage.getNumber(), withdrawalPage.getSize(), 
                withdrawalPage.getTotalElements(), withdrawalPage.getTotalPages(), withdrawalPage.isLast());
	}
